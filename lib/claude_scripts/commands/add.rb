...
module ClaudeScripts
  module Commands
    class Add < Command
      ...
      LIB_DIR = File.join(GEM_ROOT, "lib", "claude_scripts")
      COMMANDS_DIR = File.join(GEM_ROOT, "commands")
      ...
      def run
        ...
        Dir.chdir(GEM_ROOT) do
          sh "git add #{rb_file} #{md_file}"
          sh "git commit -m 'Add /#{category.tr('/', ':')}:#{name} command'"
        end

        info "Usage: /#{category.tr('/', ':')}:#{name}"
        info "Rebuild gem: gem build && gem install claude-scripts-*.gem"
      end
      ...
      def generate_md(desc, cli_cmd)
        <<~MD
          ...
          ---
          ```bash
          claude-scripts #{cli_cmd} $ARGUMENTS
          ```
        MD
      end
    end
